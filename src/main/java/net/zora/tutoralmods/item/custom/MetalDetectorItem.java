package net.zora.tutoralmods.item.custom;
        ...
        if(!pContext.getLevel().isClientSide()){
            ...
            for (int i = 0; i <=positionCliked.getY()+64;i++){
                ...
                if(isValuableBlock(state)){
                    outputValueableCoordinates(positionCliked.below(i),player, state.getBlock());
                    foundBlock = true;
                    break;
                }
            ...
            if (!foundBlock){
                player.sendSystemMessage(Component.literal("No Minerals!"));
            }
        }
